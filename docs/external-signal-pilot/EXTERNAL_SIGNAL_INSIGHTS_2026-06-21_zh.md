# External Signal Pilot 洞察报告 — 2026-06-21（中文版）
...
- **Pinterest** 提供最佳的视觉多样性与子意图发现能力。尽管登录墙导致 40/58 个 query 的结构化标签为零，但图钉集群揭示了对意图路由有价值的语义分组。

- **Canva** 专为创意/模板搜索设计，与 Curify 模板路由的可比性最高。英文 query 表现良好（36/58 成功），但中文和日本流行文化 query 几乎全部登录拦截（21/58），是需要标注的数据质量缺口。

- **消费 vs. 创作分布：** 25 个 query 偏消费导向，26 个偏创作导向，7 个混合。Curify 在消费型 query 上表现较好（完整十条结果率：82%），但在创作型 query 上明显不足（完整十条结果率：33%）——这是最迫切的成长空间。

- **Curify Gap 分析：** 5 个 P0 内容缺口（均为食谱类 query：简易工作日晚餐、无麸质餐食、备餐食谱、幼儿园自然拼读工作表、独特文化体验），15 个 P1 模板缺口，5 个 P2 召回缺口。33/58 个 query 表现正常。

- **下一步建议：** 以 Canva 的英文模板分类作为创作型 query 的路由种子；以 Bing 的相关搜索标签做类目/话题扩展；以 Pinterest 图板集群发现视觉子意图。
...
| Curify 有结果的 query | 53/58 正常，5 个空结果 |
| Google 采集完成 | 58/58 |
| Bing 采集完成 | 58/58 |
...
- Pinterest 结构化标签受登录弹窗限制，40/58 个 query 标签数为零。但全部 58 个 query 的 Top-10 结果均已采集。
- Curify 5 个空结果 query：`unique cultural experiences`、`phonics worksheets kindergarten`、`easy weeknight dinners healthy`、`gluten free dinner ideas`、`meal prep weekly recipes`。
...
Canva 的整个产品就是模板搜索——它是目前最直接的外部创作意图信号源。当 Canva 对"phonics worksheets kindergarten"返回 10 个模板而 Curify 返回 0 个时，这就是一个直接的模板缺口信号。Pinterest 的图板和图钉揭示了灵感集群：当 Pinterest 用户在搜索"monstera plant care guide infographic"时，把结果保存到"Plant care printables"、"Houseplant infographics"、"Green thumb resources"这些图板里，这些图板名称就是 Curify 可以用于路由的子意图标签。
...
|---|---|---|
| 词汇与语言 | Canva（模板分类）+ Bing（标签扩展） | 补充缺失的可打印模板；改善双语路由 |
| 角色/IP/流行文化 | Google（宽泛召回）+ Pinterest（粉丝子意图） | 确保全覆盖；用图板命名做话题扩展 |
| 生活方式与审美 | Pinterest（情绪板集群） | 无紧急行动；保持覆盖即可 |
| 艺术与设计 | Canva（模板分类） | 优先级高：这些 query 期待直接的模板输出 |
| 食物与食谱 | Canva + Pinterest | P0 缺口：紧急补充食谱海报/计划表模板 |
...
**Curify（3/4/3/4/5）：** 8-cluster 意图芯片系统提供了出色的意图清晰度。消费型覆盖尚可，但创作型 query 的完整十条结果率仅 33%——结构性内容缺口。视觉多样性受模板驱动限制，风格范围较窄。
...
**Pinterest（4/4/5/2/4）：** 视觉多样性和灵感集群最佳。登录弹窗导致 40/58 个 query 结构化标签为零，但即使没有芯片，图钉图板生态也揭示了其他平台所看不到的语义集群。对创意意图发现能力强。
...
- 57% 的 query 结果正常（33/58 Gap 分析为 P3）。

**劣势：**
- **食谱内容缺失（P0）：** `easy weeknight dinners`、`gluten free dinner ideas`、`meal prep weekly recipes`、`phonics worksheets kindergarten` 全部返回 0 条结果。这些是 Canva、Google、Pinterest 均有丰富内容的高需求创作 query。
- **创作型 query 的满十条结果率仅 33%**（30 个创作 query 中只有 10 个达到满十条）。`cuban sandwich recipe poster`、`before after kitchen organization makeover`、`lunar new year red envelope` 等 query 只返回 2–4 条结果——用户流失风险极高。
...
**优势：**
- **58/58 query 100% 返回结果**，尽管大多数 query 有登录弹窗。
- **所有平台中视觉多样性最高。** Pinterest 对任何 query 的结果集都跨越多种视觉风格、使用场景和审美方向——图板集群揭示了其他平台中看不到的语义分组。
- **对创意和消费混合型 query 效果尤佳。** 对 `creative comfort food`、`cozy reading aesthetic`、`minimalist autumn outfit for japan travel` 等 query，Pinterest 的图板聚类揭示了自然的子意图分组。
- **图板标题是隐性意图标签。** 即使没有结构化芯片，用户把某个图钉保存到的图板的标题也揭示了它服务的子意图——这个信号是 Canva 和 Google 都无法提供的。
- **灵感到创作的转化信号极强。** Pinterest 图钉混合了照片、插画、信息图和模板——结果集本身就展示了每个 query 的消费/创作比例。

**劣势：**
- **40/58 个 query 结构化标签芯片为零**，源于登录弹窗。这是 Pinterest 数据集最大的质量局限。
- **需要登录才能访问相关搜索/视觉搜索功能。** 缺少这些，Pinterest 的结构化信号大幅降低。
- **非模板原生。** Pinterest 图钉链接到外部页面；与 Canva 不同，没有"编辑此模板"入口。创意意图信号是灵感性的，不能直接用于模板路由。

**最适合的 Query 类型：** 创意灵感 query、审美 query、家居/生活方式、食物/食谱、时尚、旅行。

**Curify 应从中学到什么：**
- **用 Pinterest 图板命名惯例构建子意图分类法。** 当搜索"monstera plant care"的用户持续把结果保存到"Plant care printables"和"Houseplant infographics"这样的图板时，这些名称就是 Curify 路由应该对准的子意图集群。
- **将 Pinterest 的视觉集群多样性作为 Curify 结果多样性的对标基准。** 如果 Pinterest 对某个 query 呈现 10 种视觉风格迥异的结果，而 Curify 只呈现 10 个外观相似的模板，多样性就是问题所在。
- **Pinterest 对生活方式和审美 query 的信号质量最高**，尤其是 Google/Bing 过度返回编辑照片的场景。对 `cozy reading aesthetic` 或 `minimalist autumn outfit`，Pinterest 的结果更接近 Curify 用户想要创作的内容。
...
- **中文 query 路由不能以 Canva 为参考。** 改用 Google/Bing 标签和 Pinterest 结果。
...
| 类目/子意图分类法 | Bing Images | Google Images | Bing 每 query 40 个标签是最丰富的结构化词汇 |
| 视觉灵感集群 | Pinterest | Google Images | Pinterest 图板结构揭示语义子意图 |
| 模板/创作意图 | Canva | Pinterest | Canva 与 Curify 模板路由直接可比 |
| 中文 query 语义情报 | Google Images | Bing Images | Canva 无法访问；Pinterest 有限；Google/Bing 最适合 |
...
消费导向 ◄────────────────────────────────────► 创作导向
  ...
  Images      Images     （灵感）       （模板）    （模板）
  ★★★★★       ★★★★★      ★★★★          ★★★★        ★★★★★
```

- **Google 和 Bing** 锚定消费端。它们的价值在于召回完整性和类目广度。它们告诉你一个 query "意味着什么"，而不是"用它能做什么"。
- **Pinterest** 横跨中间地带。它面向消费者（浏览/收藏图钉），但毗邻创作（大量图钉是模板、教程和操作指南）。它是发现"灵感 → 创作"转化的最佳平台。
...
| Pinterest | 视觉风格广度 | ★★★★★ | 每个 query 的审美/风格种类最多 |
...
1. **Query 意图扩展：** 用 Google/Bing 标签识别一个 query 承载的子意图。将这些子意图映射到 Curify 的 8-cluster 系统，确保每个意图背后都有对应的模板。

2. **话题/类目路由改善：** 用 Canva 的模板类目（英文 query）和 Pinterest 的图板标题，作为 Curify 话题分类法扩展的种子词汇。每个 Canva 有但 Curify 没有对应话题 slug 的模板类目，都是路由缺口。

3. **模板匹配与库存：** P0/P1 缺口（20 个 query，34% 的 eval 集）不是路由失败——是库存失败。8-cluster 系统的路由逻辑是对的，模板不存在才是根本原因。优先级：紧急补充食谱海报/计划表模板（P0），然后是教育可打印材料变体（P1）。
...
| 严重程度 | 数量 | 示例 Query | 建议行动 |
|---|---|---|---|
| P0 — 内容缺口 | 5 | easy weeknight dinners、phonics worksheets、meal prep、gluten-free dinners、unique cultural experiences | 通过内容生成批次补充模板；立即处理 |
| P1 — 模板缺口 | 15 | cuban sandwich poster、wedding planner、bilingual flashcards、watercolor map、kitchen makeover | 为这些创作意图补充专项模板 |
| P2 — 召回缺口 | 5 | 工程、global influence、反义词、paper cutting、samurai | 审计别名和分词；内容可能已存在但未被召回 |
| P3 — 正常 | 33 | 单词、genshin、chiikawa、english-chinese、met gala | 监控回归；无需立即行动 |

---

## 9. 建议下一步

1. **修复 P0 内容缺口（立即，5 个 query）：** 启动内容生成批次，目标是食谱海报模板（`easy weeknight dinners`、`gluten free dinner ideas`、`meal prep weekly recipes`）、幼儿园自然拼读工作表和旅行文化体验模板。这些是在 Curify 零结果但外部平台信号丰富的高需求创作 query。

2. **解决 P1 模板缺口（短期，15 个 query）：** 对每个 P1 query，验证 Curify 路由在模板存在的情况下是否能正常工作。以 Canva 的 ok query 模板分类作为待补充模板的风格/格式参考。

3. **用 Bing 的 40 标签集扩展 Curify 的话题别名词汇。** 对 58 个 query 的每一个，将 Bing 相关搜索标签与 Curify 现有话题 slug 集对比。任何没有对应话题 slug 的标签集群都是分类法扩展候选项。

4. **提取 Pinterest 图板结构用于子意图发现。** 当前 pilot 采集了 Top-10 图钉但未提取图板元数据。后续应以登录状态再次访问 Pinterest，提取全部 58 个 query 的图板标题——这是目前语义子意图集群最丰富的可用来源。
...
| `docs/external-signal-pilot/EXTERNAL_SIGNAL_INSIGHTS_2026-06-21.md` | 英文版报告 |
| `docs/external-signal-pilot/EXTERNAL_SIGNAL_INSIGHTS_2026-06-21_zh.md` | 本报告（中文版） |
...
| `docs/external-signal-pilot/external-signal-5x2-summary.csv` | 5 平台 × 2 意图聚合统计 |
...
- **Pinterest 标签缺口：** 40/58 个 query 因登录弹窗结构化标签芯片为零。Pinterest 洞察依赖结果多样性观察而非结构化芯片分析。
...
| Pinterest | 58/58 结果，18/58 有标签 | 登录弹窗屏蔽结构化芯片 |
| Canva | 36/58 | 中文 + 日本流行文化 = 登录拦截 |
| Curify | 53/58 正常，5 个空结果 | 内容缺口（P0 食谱/教育类 query） |

---

*报告生成时间：2026-06-21 | 分支：baobao/multi-intent-topic-cooccurrence | Query 数：58 | 平台数：5*
