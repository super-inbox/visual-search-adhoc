...
export const CURIFY_SEARCH_EVAL_QUERIES: readonly QueryItem[] = [
...
export function assertQueries(): void {
  if (CURIFY_SEARCH_EVAL_QUERIES.length !== 58) {
    throw new Error(`Expected exactly 58 queries, got ${CURIFY_SEARCH_EVAL_QUERIES.length}`);
  }
  if (CURIFY_SEARCH_EVAL_QUERIES[0].query !== "单词") {
    throw new Error(`Unexpected first query: ${CURIFY_SEARCH_EVAL_QUERIES[0].query}`);
  }
  if (CURIFY_SEARCH_EVAL_QUERIES[57].query !== "maps") {
    throw new Error(`Unexpected last query: ${CURIFY_SEARCH_EVAL_QUERIES[57].query}`);
  }
  if (!CURIFY_SEARCH_EVAL_QUERIES.some((item) => item.query === "maps")) {
    throw new Error("maps query must be included");
  }
  const hasCat = CURIFY_SEARCH_EVAL_QUERIES.some((item) => item.query.toLowerCase() === "cat");
  if (hasCat) throw new Error("cat must not be in the eval query set");
  if (CURIFY_SEARCH_EVAL_QUERIES[34].query !== "动物 词汇") {
    throw new Error(`Query 35 must be "动物 词汇", got "${CURIFY_SEARCH_EVAL_QUERIES[34].query}"`);
  }
  if (CURIFY_SEARCH_EVAL_QUERIES[47].query !== "Spanish vocabulary printable") {
    throw new Error(`Query 48 must be "Spanish vocabulary printable", got "${CURIFY_SEARCH_EVAL_QUERIES[47].query}"`);
  }
  if (CURIFY_SEARCH_EVAL_QUERIES[48].query !== "ESL flashcards printable") {
    throw new Error(`Query 49 must be "ESL flashcards printable", got "${CURIFY_SEARCH_EVAL_QUERIES[48].query}"`);
  }
}
