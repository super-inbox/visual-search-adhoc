...
const SCREENSHOTS_BASE = path.join(
  ROOT,
  "docs/external-signal-pilot/canva-search-eval-58/screenshots",
);
...
export async function takeScreenshots(
  ...
  const file1 = path.join(dir, "canva-search-page1.png");
  await takeViewportScreenshot(page, file1);

  await page.evaluate(() => window.scrollBy(0, 900));
  await page.waitForTimeout(800);

  const file2 = path.join(dir, "canva-search-page2.png");
