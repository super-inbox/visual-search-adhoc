...
import type { CanvaObservationsFile } from "./types.js";
import {
  OBSERVATIONS_PATH,
  PROGRESS_CSV_PATH,
  DATA_DIR,
...
CANVA_SEARCH_EVAL_QUERIES.length === 58
  ? pass("Expected queries", "58")
  : fail("Expected queries", `Got ${CANVA_SEARCH_EVAL_QUERIES.length} (need 58)`);

const ids = CANVA_SEARCH_EVAL_QUERIES.map((q) => q.query_id);
...
const queryTexts = CANVA_SEARCH_EVAL_QUERIES.map((q) => q.query);
...
CANVA_SEARCH_EVAL_QUERIES[0].query === "单词"
  ? pass("First query", "单词")
  : fail("First query", `Got: ${CANVA_SEARCH_EVAL_QUERIES[0].query}`);

CANVA_SEARCH_EVAL_QUERIES[57].query === "maps"
  ? pass("Last query", "maps")
  : fail("Last query", `Got: ${CANVA_SEARCH_EVAL_QUERIES[57].query}`);

CANVA_SEARCH_EVAL_QUERIES.some((q) => q.query === "maps")
  ? pass("maps query included")
  : fail("maps query included", "maps not found");

!CANVA_SEARCH_EVAL_QUERIES.some((q) => q.query.toLowerCase() === "cat")
  ? pass("cat excluded")
  : fail("cat excluded", "cat was found in queries");

CANVA_SEARCH_EVAL_QUERIES[34].query === "动物 词汇"
  ? pass("Query 35", "动物 词汇")
  : fail("Query 35", `Got: "${CANVA_SEARCH_EVAL_QUERIES[34].query}"`);

CANVA_SEARCH_EVAL_QUERIES[47].query === "Spanish vocabulary printable"
  ? pass("Query 48", "Spanish vocabulary printable")
  : fail("Query 48", `Got: "${CANVA_SEARCH_EVAL_QUERIES[47].query}"`);

CANVA_SEARCH_EVAL_QUERIES[48].query === "ESL flashcards printable"
  ? pass("Query 49", "ESL flashcards printable")
  : fail("Query 49", `Got: "${CANVA_SEARCH_EVAL_QUERIES[48].query}"`);

// ── Load observations ─────────────────────────────────────────────────────────

let obs: CanvaObservationsFile | null = null;
const obsExists = fs.existsSync(OBSERVATIONS_PATH);

if (!obsExists) {
  warn("observations.json", "File does not exist yet — run collect:canva-search first");
} else {
  try {
    obs = JSON.parse(fs.readFileSync(OBSERVATIONS_PATH, "utf8")) as CanvaObservationsFile;
...
if (obs) {
  ...
  const orderMatch = obs.queries.every(
    (q, i) => q.query_id === CANVA_SEARCH_EVAL_QUERIES[i].query_id,
  );
  ...
  for (let i = 0; i < obs.queries.length; i++) {
    const expected = CANVA_SEARCH_EVAL_QUERIES[i];
  ...
  for (const q of obs.queries) {
    ...
    if (q.status === "ok" || q.status === "partial") {
      ...
      if (!q.screenshots.page1 || !q.screenshots.page2) {
        ...
        if (!q.screenshots.page1.endsWith("canva-search-page1.png")) {
          fail(`Query ${q.query_id} screenshot filename`, "page1 must end with canva-search-page1.png");
        }
        if (!q.screenshots.page2.endsWith("canva-search-page2.png")) {
          fail(`Query ${q.query_id} screenshot filename`, "page2 must end with canva-search-page2.png");
        }
      }
    }
  }
  ...
  // Summary
  console.log("\nCanva Search Evaluation — 58-query set\n");
  ...
  console.log(`\nOverall: ${overall}`);
  console.log("Run collect:canva-search first to generate observation data.\n");
