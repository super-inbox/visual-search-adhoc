...
const SCREENSHOTS_BASE = path.join(
  ROOT,
  "docs/external-signal-pilot/pinterest-search-eval-58/screenshots",
);
...
async function takeViewportScreenshot(page: Page, filePath: string): Promise<void> {
...
export async function takeScreenshots(
  ...
  const file1 = path.join(dir, "pinterest-search-page1.png");
  await takeViewportScreenshot(page, file1);

  await page.evaluate(() => window.scrollBy(0, 900));
  await page.waitForTimeout(800);

  const file2 = path.join(dir, "pinterest-search-page2.png");
  await takeViewportScreenshot(page, file2);

  await page.evaluate(() => window.scrollTo(0, 0));

  return { page1: file1, page2: file2 };
}
