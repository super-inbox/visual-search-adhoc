...
const PROFILE_DIR = path.join(ROOT, ".cache/google-image-eval-profile");
...
export async function createBrowserContext(headless = false): Promise<BrowserContext> {
  ...
  // Try system Chrome first — avoids Playwright Chromium install issues
  try {
...
export function buildSearchUrl(query: string): string {
  return `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(query)}&hl=en`;
}
