...
BING_IMAGE_EVAL_QUERIES.length === 58
  ? pass("Expected queries", "58")
  : fail("Expected queries", `Got ${BING_IMAGE_EVAL_QUERIES.length} (need 58)`);

const ids = BING_IMAGE_EVAL_QUERIES.map((q) => q.query_id);
...
const queryTexts = BING_IMAGE_EVAL_QUERIES.map((q) => q.query);
...
BING_IMAGE_EVAL_QUERIES[0].query === "单词"
  ? pass("First query", "单词")
  : fail("First query", `Got: ${BING_IMAGE_EVAL_QUERIES[0].query}`);

BING_IMAGE_EVAL_QUERIES[57].query === "maps"
  ? pass("Last query", "maps")
  : fail("Last query", `Got: ${BING_IMAGE_EVAL_QUERIES[57].query}`);

BING_IMAGE_EVAL_QUERIES.some((q) => q.query === "maps")
  ? pass("maps query included")
  : fail("maps query included", "maps not found");

!BING_IMAGE_EVAL_QUERIES.some((q) => q.query.toLowerCase() === "cat")
  ? pass("cat excluded")
  : fail("cat excluded", "cat was found in queries");

BING_IMAGE_EVAL_QUERIES[34].query === "动物 词汇"
  ? pass("Query 35", "动物 词汇")
  : fail("Query 35", `Got: "${BING_IMAGE_EVAL_QUERIES[34].query}"`);

BING_IMAGE_EVAL_QUERIES[47].query === "Spanish vocabulary printable"
  ? pass("Query 48", "Spanish vocabulary printable")
  : fail("Query 48", `Got: "${BING_IMAGE_EVAL_QUERIES[47].query}"`);

BING_IMAGE_EVAL_QUERIES[48].query === "ESL flashcards printable"
  ? pass("Query 49", "ESL flashcards printable")
  : fail("Query 49", `Got: "${BING_IMAGE_EVAL_QUERIES[48].query}"`);

// ── Load observations ─────────────────────────────────────────────────────────

let obs: BingObservationsFile | null = null;
const obsExists = fs.existsSync(OBSERVATIONS_PATH);

if (!obsExists) {
  warn("observations.json", "File does not exist yet — run collect:bing-images first");
} else {
  try {
    obs = JSON.parse(fs.readFileSync(OBSERVATIONS_PATH, "utf8")) as BingObservationsFile;
...
if (obs) {
  ...
  const orderMatch = obs.queries.every((q, i) => q.query_id === BING_IMAGE_EVAL_QUERIES[i].query_id);
  ...
  for (let i = 0; i < obs.queries.length; i++) {
    const expected = BING_IMAGE_EVAL_QUERIES[i];
  ...
  const statusCounts = { ok: 0, ok_empty: 0, blocked: 0, error: 0, partial: 0, pending: 0, running: 0 };
    ...
    if (q.status === "ok" || q.status === "partial") {
      ...
      if (!q.screenshots.page1 || !q.screenshots.page2) {
        ...
        if (!fs.existsSync(q.screenshots.page1)) {
          fail(`Query ${q.query_id} screenshot page1`, `Missing file: ${q.screenshots.page1}`);
        }
        if (!fs.existsSync(q.screenshots.page2)) {
          fail(`Query ${q.query_id} screenshot page2`, `Missing file: ${q.screenshots.page2}`);
        }
        // Validate filenames
        if (!q.screenshots.page1.endsWith("bing-images-page1.png")) {
          fail(`Query ${q.query_id} screenshot filename`, `page1 must end with bing-images-page1.png`);
        }
        if (!q.screenshots.page2.endsWith("bing-images-page2.png")) {
          fail(`Query ${q.query_id} screenshot filename`, `page2 must end with bing-images-page2.png`);
        }
      }
    }
  }
  ...
  missingScreenshots.length === 0
    ...
    : warn("Missing screenshots", `${missingScreenshots.length} queries: ${missingScreenshots.slice(0, 5).join(", ")}`);

  // per-query JSON files
  let missingPerQuery = 0;
  if (fs.existsSync(PER_QUERY_DIR)) {
    for (const q of obs.queries) {
      if (q.status === "ok" || q.status === "ok_empty" || q.status === "partial") {
  ...
  // Print summary
  console.log("\nBing Images Evaluation — 58-query set\n");
  ...
  const allDone = obs.queries.every((q) => ["ok", "ok_empty", "blocked", "error"].includes(q.status));
  ...
  // No obs file yet — just print check results
  console.log("\n--- Check Results ---");
  for (const r of results) {
    const icon = r.status === "PASS" ? "✓" : r.status === "WARN" ? "⚠" : "✗";
    const detail = r.detail ? ` — ${r.detail}` : "";
  ...
  console.log(`\nOverall: ${overall}`);
  console.log("Run collect:bing-images first to generate observation data.\n");

  fs.mkdirSync(DATA_DIR, { recursive: true });
  fs.writeFileSync(
    VALIDATION_REPORT_PATH,
    JSON.stringify({ generated_at: new Date().toISOString(), overall, checks: results, warnings: warns, summary: null }, null, 2),
    "utf8",
  );
  console.log(`Report saved to: ${VALIDATION_REPORT_PATH}`);
}
