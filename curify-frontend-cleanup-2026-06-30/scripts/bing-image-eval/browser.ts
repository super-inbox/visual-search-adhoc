...
const PROFILE_DIR = path.join(ROOT, ".cache/bing-image-eval-profile");
...
export function buildBingImagesUrl(query: string): string {
  return `https://www.bing.com/images/search?q=${encodeURIComponent(query)}&mkt=en-US&setlang=en-US&safeSearch=Moderate`;
}
